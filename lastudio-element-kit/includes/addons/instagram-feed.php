    ...
    protected function enqueue_addon_resources(){
	    ...
	    if(!lastudio_kit_settings()->is_combine_js_css()) {
		    ...
		    if(!lastudio_kit()->is_optimized_css_mode()) {
			    wp_register_style( $this->get_name(), lastudio_kit()->plugin_url( 'assets/css/addons/instagram-feed.min.css' ), [ 'lastudio-kit-base', 'e-swiper'], lastudio_kit()->get_version() );
...
//                    'hashtag'  => esc_html__( 'Tagged Photos', 'lastudio-kit' ),
            ...
            foreach ( $posts as $post_data ) {
                ...
                if ( $this->config['post_link'] ) {
                    ...
                    if ( 'lightbox' === $this->config['post_link_type'] ) {
                        ...
                        $link = $img_data['src'];
                        $attr = ' data-elementor-open-lightbox="yes" data-elementor-lightbox-slideshow="' . esc_attr($this->get_id()) . '"';
                    }
    ...
    public function print_notice( $notice ) {
        ...
        printf( '<div class="lakit-instagram-feed__notice">%s</div>', esc_html($notice) );
    }
