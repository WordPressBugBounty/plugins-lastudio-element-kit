...
class LaStudioKit_Tabs extends LaStudioKit_Base {
  ...
  protected function render() {
    ...
    ?>
    <div <?php $this->print_render_attribute_string( 'instance' ); ?>>
      <div class="lakit-tabs__control-wrapper">
        ...
        if ( $tab_as_dropdown ) {
          ...
          if ( ! empty( $intro_text ) ) {
            echo sprintf( '<div class="intro-text">%1$s</div>', esc_html($intro_text) );
          }
        ...
        if ( $tab_as_dropdown ) {
          ...
          echo '</div>';
          echo sprintf( '<div class="lakit-tabs__controls__tmp"><div class="lakit-tabs__controls__text"></div>%1$s</div>', $icon_dd ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
          echo '</div>';
          echo '</div>';
        } else {
      ...
      <div class="lakit-tabs__content-wrapper">
        ...
        foreach ( $tabs as $index => $item ) {
                  ...
                  if ( $item_enable_ajax && ! Plugin::instance()->editor->is_edit_mode() ) {
                    $template_content = '<div data-lakit_ajax_loadtemplate="true" data-template-id="' . esc_attr( $item['item_template_id'] ) . '"><span class="lakit-css-loader"></span></div>';
                      ...
                      if(Plugin::instance()->editor->is_edit_mode()){
                          ...
                          echo sprintf('<link rel="stylesheet" id="elementor-post-%1$s-css" href="%2$s" type="text/css" media="all" />', esc_attr($item['item_template_id']), esc_url($css_file->get_url()) );
                      }
                      echo Plugin::$instance->frontend->get_builder_content( esc_attr($item['item_template_id']), false ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
          ...
          echo sprintf( '<div %1$s>%2$s</div>', $this->get_render_attribute_string( $tab_content_setting_key ), $content_html ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        }
        ?>
      </div>
    </div>
