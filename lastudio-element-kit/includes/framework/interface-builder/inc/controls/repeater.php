...
if ( ! class_exists( 'CX_Control_Repeater' ) ) {
	...
	class CX_Control_Repeater extends CX_Controls_Base {
		...
		public function print_js_template() {
			...
			echo $this->get_js_template(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

			if ( ! empty( $this->_childs ) ) {
				foreach ( $this->_childs as $child ) {
					echo $child->get_js_template(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
			}
