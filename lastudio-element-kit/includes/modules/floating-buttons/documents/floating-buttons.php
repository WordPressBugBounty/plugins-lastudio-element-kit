...
namespace LaStudioKitThemeBuilder\Modules\FloatingButtons\Documents;

use Elementor\Modules\Library\Traits\Library as Library_Trait;
use LaStudioKitThemeBuilder\Modules\FloatingButtons\Module as Floating_Buttons_Module;
...
class Floating_Buttons extends Theme_Document {
	...
	public static function get_properties() {
		...
		$properties['library_close_title'] = esc_html__( 'Go To Dashboard', 'lastudio-kit' );
		$properties['publish_button_title'] = esc_html__( 'After publishing this widget, you will be able to set it as visible on the entire site in the Admin Table.', 'lastudio-kit' );
	...
	public function add_built_with_elementor( $actions ) {
		if ( $this->is_built_with_elementor() && $this->is_editable_by_current_user() ) {
			...
			$actions['edit_with_elementor'] = sprintf(
				'<a href="%1$s">%2$s</a>',
				$edit_url,
				esc_html__( 'Edit with Elementor', 'lastudio-kit' )
			);
		}
	...
	public static function get_title() {
		return esc_html__( 'Floating Element', 'lastudio-kit' );
	}

	public static function get_plural_title() {
		return esc_html__( 'Floating Elements', 'lastudio-kit' );
	}
